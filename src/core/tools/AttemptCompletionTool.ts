...
export class AttemptCompletionTool extends BaseTool<"attempt_completion"> {
	...
	async execute(params: AttemptCompletionParams, task: Task, callbacks: AttemptCompletionCallbacks): Promise<void> {
		...
		try {
			...
			// Force a final token usage update here, once per model-initiated
			// attempt_completion call, so the latest stats reach the UI regardless of
			// throttle. Skipped for a stale history replay (revisiting an
			// already-completed subtask) since that reruns this handler on a fresh Task
			// instance.
