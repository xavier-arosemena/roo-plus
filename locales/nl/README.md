<div align="center">
<sub>

[English](../../README.md) • [Català](../ca/README.md) • [Deutsch](../de/README.md) • [Español](../es/README.md) • [Français](../fr/README.md) • [हिंदी](../hi/README.md) • [Bahasa Indonesia](../id/README.md) • [Italiano](../it/README.md) • [日本語](../ja/README.md)

</sub>
<sub>

[한국어](../ko/README.md) • <b>Nederlands</b> • [Polski](../pl/README.md) • [Português (BR)](../pt-BR/README.md) • [Русский](../ru/README.md) • [Türkçe](../tr/README.md) • [Tiếng Việt](../vi/README.md) • [简体中文](../zh-CN/README.md) • [繁體中文](../zh-TW/README.md)

</sub>
</div>

# Roo+

<p align="center">
  <a href="https://open-vsx.org/extension/xavier-arosemena/roo-plus">
    <img src="https://img.shields.io/badge/Open_VSX_Registry-007ACC?style=flat&logo=eclipse&logoColor=white" alt="Open VSX Registry">
  </a>
  <a href="https://marketplace.visualstudio.com/items?itemName=xavier-arosemena.roo-plus">
    <img src="https://img.shields.io/badge/VS_Code_Marketplace-0078D4?style=flat&logo=visualstudiocode&logoColor=white" alt="VS Code Marketplace">
  </a>
  <a href="https://github.com/xavier-arosemena/roo-plus/issues">
    <img src="https://img.shields.io/badge/Report_Bug-GitHub-181717?style=flat&logo=github&logoColor=white" alt="Report Bug">
  </a>
</p>

Roo+ is een **fork van Zoo Code**: een AI-ontwikkelassistent met **90 vooraf geladen aangepaste modi** uit een catalogus van **301 modi**, plus **MCP-ondersteuning** en **meer dan 25 providers**.

## Installatie

1. Open **VS Code** of **VS Codium**.
2. Ga naar **Extensies** (`Ctrl+Shift+X` / `Cmd+Shift+X`).
3. Zoek naar **"Roo+"**.
4. Klik op **Installeren**.

Of installeer het rechtstreeks vanaf de opdrachtregel:

```bash
code --install-extension xavier-arosemena.roo-plus
ovsx install xavier-arosemena/roo-plus
```

Of bouw het vanuit de broncode door de sectie [Snel aan de slag](../../README.md#quick-start) van de Engelse README te volgen.

## Bronnen

- **[Volledige README (Engels)](../../README.md)** — Volledige documentatie van Roo+.
- **[Bijdragen](./CONTRIBUTING.md)** — Handleiding om aan het project bij te dragen.
- **[GitHub Issues](https://github.com/xavier-arosemena/roo-plus/issues)** — Meld bugs en volg de ontwikkeling.
- **[GitHub Discussions](https://github.com/xavier-arosemena/roo-plus/discussions)** — Deel ideeën en neem deel aan de community.

---

**Veel plezier met Roo+!** Heb je vragen of ideeën voor functies, open dan een [issue](https://github.com/xavier-arosemena/roo-plus/issues) of start een [discussie](https://github.com/xavier-arosemena/roo-plus/discussions).
