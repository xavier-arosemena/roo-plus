...
| Mode descriptions blank/missing in shipped VSIX                        | Enforced at every layer: shared `getModeDisplayDescription()` fallback (`whenToUse` → `roleDefinition`), `CustomModesManager` normalization, `modeConfigSchema` `min(1)` + `roomodes.json` `minLength: 1`, `ensure_descriptions.py` CI gate, submodule-pin pre-build gate | Unreleased |
| Marketplace installs dropped full mode descriptions                    | `SimpleInstaller` preserves the full description from marketplace `content` (regression-tested)                                                                                                                                                                           | Unreleased |
| Semble downloader trusted a cached broken binary (R1)                  | `isSembleBinaryHealthy()` bounded `--help` probe before cache reuse; falls through to a re-download on failure                                                                                                                                                            | Unreleased |
| Semble sticky permanent "Error" on transient search failure (R2)       | Per-call throw; shared state stays `Indexed`; `Error` reserved for install-time failures                                                                                                                                                                                  | Unreleased |
| Semble cold-start misreported as broken index (R3/R4)                  | "Indexed" ready message includes embedding-model download hint (`semble.firstSearchDownloadsModel` i18n, 18 locales)                                                                                                                                                      | Unreleased |
| `verify-roomodes-sync.mjs` silently skipped a missing submodule        | Hard-fails when the `custom-modes` submodule is not initialized (packaging hazard)                                                                                                                                                                                        | Unreleased |
| No release governance for the Semble binary                            | `docs/SEMBLE-RELEASE-GOVERNANCE.md` (immutable tags) + `verify-semble-release-coupling.mjs` CI gate + installer-side `sembleexec-release-guard.yml`                                                                                                                       | Unreleased |
| Code-index core drift vs upstream unguarded                            | `verify-upstream-code-index-alignment.mjs` CI gate keeps the Qdrant core byte-identical to upstream Zoo-Code                                                                                                                                                              | Unreleased |
