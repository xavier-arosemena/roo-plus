...
### 6. Outbound `ExtensionMessage` Fully Typed/Validated (Phase 2 ✅ DELIVERED)

**Location**: [`packages/types/src/vscode-extension-host.ts`](packages/types/src/vscode-extension-host.ts)
**Issue**: RESOLVED — the outbound `ExtensionMessage` union (77 types) has received the full discriminated-union/zod treatment as `WebviewMessage`. Phase 0 scaffolded `parseExtensionMessage` + `extensionMessageSchemas` ([`packages/types/src/extension-messages/index.ts`](packages/types/src/extension-messages/index.ts)) with 5 baseline types registered, then Phase 2 Domains 1-7 landed 69 more across UI/navigation, model/status, task/chat/history, checkpoint/modes, marketplace, code-index and worktree responses. Phase 2, Domain 8 (skills/rules/history-import responses — the final domain) landed the last 3 (`skills`, `rules`, `rooHistoryImportProgress`), all with active outbound producers (`src/core/webview/skillsMessageHandler.ts`, `rulesMessageHandler.ts`, `handlers/misc.ts`) and typed schemas in [`packages/types/src/extension-messages/skillsRules.ts`](packages/types/src/extension-messages/skillsRules.ts) (reusing the `SkillMetadata`/`RuleMetadata` shapes and mirroring the inline `rooHistoryImportProgress` payload). The outbound ratchet (`EXTENSION_MESSAGE_BASELINE`) now covers **77 registered / 0 untyped** (`UNTYPED_EXTENSION_MESSAGE_LIMIT = 0`). Vestigial outbound types (no outbound producer, registered for ratchet completeness): Domain 2 `vsCodeLmApiAvailable`, `authenticatedUser`; Domain 3 `organizationSwitchResult`; Domain 4 `updateCustomMode`, `deleteCustomMode` (direction-mixed — the webview SENDS these inbound); Domain 5 `shareTaskSuccess` (direction-mixed — the webview SENDS this inbound, registered outbound minimally; the OUTBOUND registration is authoritative and the inbound minimal schema is kept pending the Phase 3 direction-mixing cleanup); Domain 6 `codebaseIndexConfig` (vestigial — no outbound producer; the config is carried inside the `state` payload).
**Impact**: Full schema-driven type safety on the extension→webview|CLI direction; every one of the 77 outbound `ExtensionMessage` types is strictly validated at the `parseExtensionMessage` boundary (webview `ExtensionStateContext`/`App`, CLI `message-processor`), closing the outbound "Type Lie".
**Suggested Fix**: (Complete — Phase 2 outbound migration done.) Follow-up cleanup for Phase 3: the direction-mixing cleanup (moving outbound-only types out of `WebviewMessage` and dropping the now-vestigial transitional pass-through paths).
