...
| React Compiler         | Disabled — removed in the issue #250 fix; production compiles like the vitest suite (no compiler). **Enforced** by the toolchain-parity gate (section 11): test and production MUST compile with identical Babel/React tooling. | [`webview-ui/vite.config.ts`](webview-ui/vite.config.ts:87) · [`webview-ui/scripts/verify-webview-build-parity.mjs`](webview-ui/scripts/verify-webview-build-parity.mjs) |
...
## 11. Toolchain parity invariant (systemic quality gate — issue #250)

**Invariant (non-negotiable):** the webview MUST be compiled with **identical Babel/React tooling** in the test path (vitest JSDOM suite and Playwright CT) and the production path (`vite build`). A passing test suite is only meaningful if it proves the artifact that actually ships.

**Why it exists:** issue #250 was a toolchain **divergence**, not a code bug. `babel-plugin-react-compiler` was enabled in the production [`webview-ui/vite.config.ts`](webview-ui/vite.config.ts:87) but absent from the vitest suite, so the 1,629-test suite stayed green while the shipped bundle compiled the render-phase "adjust state during render" guards with the React Compiler and looped into React error #301 ("Too many re-renders") at startup. Fix #1 removed the compiler from the production build; this section defines how that parity is **enforced** so the divergence can never silently ship again.

**Enforcement:**

- **Config parity gate** — [`webview-ui/scripts/verify-webview-build-parity.mjs`](webview-ui/scripts/verify-webview-build-parity.mjs) compares the `@vitejs/plugin-react` `react(...)` plugin configuration declared in `vite.config.ts`, `playwright-ct.config.ts`, and `vitest.config.ts` and fails on any divergence. It also hard-fails if `babel-plugin-react-compiler` / `react-compiler` appears in any of the three toolchain configs.
- **Bundle canary** — the same script rebuilds the production bundle (`vite build`) and fails if the output contains React Compiler runtime markers (`react-compiler-runtime`, `useMemoCache`), which only appear when production compiles differently from the test path. (`jsxDEV` is reported but deliberately NOT a hard gate: it appears in legitimate compiler-free bundles from third-party JSX-transform runtime `development` branches, e.g. `hast-util-to-jsx-runtime`.)
- **CI** — the gate runs on every PR and master push as the `Verify webview build/test toolchain parity (issue #250 gate)` step in the `compile` job of [`.github/workflows/code-qa.yml`](.github/workflows/code-qa.yml), and its unit tests run under `pnpm test:scripts`.
- **Runtime regression test** — [`webview-ui/src/__tests__/App.spec.tsx`](webview-ui/src/__tests__/App.spec.tsx:599) ("mounts without render-looping for an existing user with an announcement pending (issue #250)") mounts `AppWithProviders` with `shouldShowAnnouncement=true` and a configured `apiConfiguration` (API key present → `showWelcome=false`) and asserts no "Too many re-renders" / "Maximum update depth exceeded" is logged.

**Operational rule:** never enable the React Compiler (or any Babel plugin) in only one webview toolchain. Any Babel/React tooling change must be applied identically to all three configs (`vite.config.ts`, `playwright-ct.config.ts`, `vitest.config.ts`) and must keep this gate green.
