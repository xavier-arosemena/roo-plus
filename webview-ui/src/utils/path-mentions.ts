...
export function convertToMentionPath(path: string, cwd?: string): string {
	...
	if (lowerPath.startsWith(lowerCwd)) {
		...
		// Escape backslashes and spaces in the path with backslashes, then
		// HTML-encode the result. `escapeHtml` must run last so HTML special
		// characters (`<`, `>`, `&`, `'`) are entity-encoded rather than
		// shell-escaped; the shell-sensitive `escapeSpaces` is reserved for
		// query contexts.
		const escapedRelativePath = relativePath.replace(/\\/g, "\\\\").replace(/ /g, "\\ ")
