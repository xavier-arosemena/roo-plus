...
Starting the 3.87 pre-release line. Milestone two begins now — guided by your feedback, we're doubling down on building a local-first, agentic IDE extension that makes your AI spend legible and your knowledge portable.
...
First stable release on the 3.85 line, promoting the features shipped and tested on the 3.84.x pre-release channel. Bumping to 3.85.0 also corrects the marketplace versioning so the stable outranks the 3.84.x pre-releases on Open VSX.

### 🚀 Enhancements

- **Red-team mode added to the Mode Marketplace** — The canonical catalog grows to 302 modes (90 pre-loaded) with the addition of Red Team Mode, a structured, constructive adversary that stress-tests proposals (#255).
...
- **What's New popup re-arms per version** — The announcement popup's identity is now derived from the extension version so it reliably reappears after every version bump (#265).
...
- **Welcome screen tuned (Issue #196)** — [`RooHero.tsx`](webview-ui/src/components/welcome/RooHero.tsx) replaces the animated SVG-mask outback scene with a pure-ASCII figlet **"Roo+" wordmark** (font-safe, theme-aware, renders in the theme foreground color — no light/dark artwork pair); [`RooTips.tsx`](webview-ui/src/components/welcome/RooTips.tsx) forces a line break in the intro ("Your multi-agent AI team — <br>301 specialized modes…") via `Trans` + `<br/>` across all 18 locales and keeps the icon + label on one line (`whitespace-nowrap` inner flex wrapper). A preview page ([`roo-hero-compare.html`](webview-ui/roo-hero-compare.html)) was added as a design reference.

### 🐛 Bug Fixes

- **Webview react-hooks v7 lint errors resolved (Closes: #221)** — All 29 react-hooks v7 violations across the webview were fixed (previously masked by the broken `code-qa.yml` and committed with `--no-verify`): `App.tsx`, `ChatView.tsx`, `CodeIndexPopover.tsx`, `ContextMenu.tsx`, `FollowUpSuggest.tsx`, `McpExecution.tsx`, `ReasoningBlock.tsx`, `UpdateTodoListToolBlock.tsx`, `MarketplaceView.tsx`, `ModesView.tsx`, settings providers, `usePromptHistory`, `useScrollLifecycle`, and more now pass `eslint --max-warnings=0`.
- **Webview regression fixes from the react-hooks v7 refactor (Closes: #221)** — Four regressions introduced by the `f8505866f` refactor are fixed: [`FollowUpSuggest.tsx`](webview-ui/src/components/chat/FollowUpSuggest.tsx) countdown now initializes on mount (sentinel-initialized previous-key state); [`usePromptHistory.ts`](webview-ui/src/components/chat/hooks/usePromptHistory.ts) now populates prompt history on first render (sentinel-initialized previous-history state); [`useScrollLifecycle.ts`](webview-ui/src/hooks/useScrollLifecycle.ts) keeps the phase ref in sync during render so transient hydration-time not-at-bottom signals no longer reveal the scroll-to-bottom CTA; [`OpenAICodexRateLimitDashboard.tsx`](webview-ui/src/components/settings/providers/OpenAICodexRateLimitDashboard.tsx) shows its loading state when authenticated on mount.
- **`FileChangesPanel` postMessage origin verification (CodeQL #60)** — The `fileContent` response listener in [`FileChangesPanel.tsx`](webview-ui/src/components/chat/FileChangesPanel.tsx) now verifies messages with `isTrustedMessage(event)` before applying content (closes the last unguarded consumer tracked in DEBT #15), with a 183-line regression spec.
- **e2e log-injection sanitizer hardened (CodeQL #112)** — [`mcp-oauth.test.ts`](apps/vscode-e2e/src/suite/mcp-oauth.test.ts) sanitizes log output so injected ANSI/control sequences cannot corrupt CI logs.

### 🛡️ Security

- **Security policy professionalized (Closes: #234)** — [`SECURITY.md`](SECURITY.md) rewritten: GitHub **Private Vulnerability Disclosure** is the only supported reporting channel (never public issues), a concrete supported-version table (latest stable fully supported, previous minor best effort), a structured report template, SLAs, coordinated-disclosure expectations, scope, and a documented security posture. Path-based [`CODEOWNERS`](.github/CODEOWNERS) now requires maintainer review for `SECURITY.md`, `dependabot.yml`, `codeql.yml`, and the `security-alert-*` workflows, and a new [`security-issue-assign.yml`](.github/workflows/security-issue-assign.yml) auto-assigns maintainers when the `security` label is applied.
- **Dependabot alerts + quality issues resolved (Closes: #229)** — CodeQL findings fixed (FileChangesPanel origin check, e2e log-injection sanitizer); three new workflows automate security hygiene: [`dependabot-auto-merge.yml`](.github/workflows/dependabot-auto-merge.yml) (CI-gated auto-merge, skips semver-major bumps for human review), [`security-alert-digest.yml`](.github/workflows/security-alert-digest.yml) (nightly security & quality digest issue), and [`security-alert-issue.yml`](.github/workflows/security-alert-issue.yml) (auto-opens issues for high/critical alerts).

### 🔧 Chores

- **Stable release & pre-release channel optimized (Closes: #221)** — Release plumbing consolidated: a new composite action [`prepare-extension/action.yml`](.github/actions/prepare-extension/action.yml) centralizes pre-flight (submodule pin, `.roomodes` sync, package identity, publish ref, marketplace tokens); [`marketplace-publish.yml`](.github/workflows/marketplace-publish.yml) reworked; the old `nightly-publish.yml` became [`pre-release-publish.yml`](.github/workflows/pre-release-publish.yml) for the "pre-release first" flow; [`release-validation.yml`](.github/workflows/release-validation.yml) trimmed. The full lifecycle is documented in the new [`docs/RELEASE-WORKFLOWS.md`](docs/RELEASE-WORKFLOWS.md).
- **CI: type-check restored after incompatible dependency bumps (Closes: #221, #229)** — Two dependabot bumps merged on `origin/master` broke `pnpm check-types`: `@ai-sdk/openai-compatible@3.0.x` pulled `@ai-sdk/provider@4` (returning `LanguageModelV4`) while the pinned `ai@6.0.218` only supports V2/V3, and `google-auth-library@^11` conflicted with `@anthropic-ai/vertex-sdk`'s required `^10.2.0`. Reverted to `@ai-sdk/openai-compatible@2.0.56` and `google-auth-library@^10.2.0` in [`src/package.json`](src/package.json) (+ lockfile), restoring a single compatible dependency tree.
- **`.github` templates simplified and rebranded to Roo+ (Closes: #222)** — Issue templates converted from Markdown to YAML forms (`bug_report.yml`, `feature_request.yml`, `add-provider-model.yml`) with a `config.yml`; the PR template was simplified; the stale `release-reminder.yml` workflow was removed; remaining `main`-branch and upstream references updated.
- **Localized READMEs rewritten as Roo+ landing pages (Closes: #220)** — All 17 `locales/*/README.md` files (previously stale translations of the upstream Zoo Code README) are now compact Roo+-only landing pages: language switcher, Open VSX + GitHub badges, localized intro, install instructions, and links to the English README / CONTRIBUTING.md. All upstream references removed; root [`README.md`](README.md) gained the language-switcher bar; CONTRIBUTING.md upstream links fixed (root + 17 locales). A new CI guard [`scripts/verify-locale-readmes.mjs`](scripts/verify-locale-readmes.mjs) (14 spec tests) enforces no upstream markers and required Roo+ content, wired into `release-validation.yml` and `test:scripts`. Policy: locale READMEs are lightweight landing pages; the English README is the single canonical document (DEBT #8 resolved).
- **`docs/` reorganized + ADR protocol established (Closes: #224)** — Documentation is now split by purpose: `src/docs/` (permanent ADRs + architecture reviews), `docs/` (living operational docs), `plans/` (temporal plans/execution records, gitignored). New [`src/docs/README.md`](src/docs/README.md) documents the protocol, [`src/docs/ADR-TEMPLATE.md`](src/docs/ADR-TEMPLATE.md) provides the template, and three new ADRs were recorded: [`adr-shared-types-package.md`](src/docs/adr-shared-types-package.md), [`adr-upstream-alignment-fork-strategy.md`](src/docs/adr-upstream-alignment-fork-strategy.md), and [`adr-webview-handler-decomposition.md`](src/docs/adr-webview-handler-decomposition.md). Stale hand-off/arch-review/`release-changes-since-3.72.0.md` files were retired from `docs/`.
- **`DEBT.md` audited against current code (Closes: #225)** — Five new debt items inferred from a code audit (#17 CLI rebrand incomplete / upstream upgrade path, #18 extension-side monoliths `Task.ts`/`McpHub.ts`/`bedrock.ts`, #19 webview i18n not CI-enforced + `find-missing-i18n-key.js` false positives, #20 stale submodule-pin references in ADRs, #21 `PKG_OUTPUT_CHANNEL` `"Zoo-Code"` rebrand residual); existing items refreshed (#4 quantified, #11 `as any` snapshot 142 src / 41 webview / 9 `@ts-ignore`, #15 partially resolved via FileChangesPanel guard, #16 distribution half closed via GitHub Release tarballs) and the TODO/FIXME inventory expanded.
