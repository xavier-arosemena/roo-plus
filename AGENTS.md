...
Only `master` is long-lived. Full policy, audit commands, and the deletion decision tree: [`docs/runbooks/branch-hygiene.md`](docs/runbooks/branch-hygiene.md).

- Branch from current `master`; never commit directly on `master` (pull with `git pull --ff-only`).
- Delete the branch locally **and** on `origin` as soon as its PR merges or closes as superseded. Do not leave merged or duplicate refs behind.
- Never re-land the same work twice: if a PR closes unmerged, rebranch from current `master` and link the closed PR. Do not resurrect the old branch with a force-push.
- Evidence for deleting an unmerged branch is required: `git cherry -v origin/master <branch>` for patch identity plus a blob comparison of the touched files against `origin/master`.
- Do not create persistent `release/*` branches or out-of-band publish workflows. Publishing, including the Open VSX pre-release guard, is owned by [`.github/workflows/pre-release-publish.yml`](.github/workflows/pre-release-publish.yml:1) on `master`; `release/v3.88.0-prerelease` and `ovsx-prerelease-publish.yml` were retired on 2026-09-23.
- Do not park work in `git stash`. Stashes created from dependency-bot branches are throwaway; use a short-lived `wip/*` branch if work must persist.

## Release Versioning Policy

The extension follows the **iterate-then-stabilize, same-minor** release policy (full rationale: [ADR: Unified Release Versioning](docs/adr/adr-release-versioning-policy.md)). These rules are the source of truth for any version/announcement work:

- **Internal == publishing, always.** The version in `src/package.json` IS the version shipped for every build. Never derive or mutate the version number at build/CI time — for pre-release OR stable.
- **Pre-releases are committed consecutive patches on the current minor** (`3.88.0`, `3.88.1`, `3.88.2`, …). Bump with `pnpm bump:pre-release`.
- **The stable is the NEXT PATCH on that same minor** (`3.88.3`) — it promotes the tested bits. It is NOT a new minor.
- **The next release cycle starts a new minor** (`3.89.0`, via `pnpm bump:line`).
- **One CHANGELOG section per minor**, titled `## [<major>.<minor>.0]`. Never create per-patch sections.
- **Announcements are keyed to the LINE BASE `<major>.<minor>.0`.** Any patch on the minor resolves to it at runtime via `getAnnouncementForVersion`, and the popup arms ONCE PER MINOR (`latestAnnouncementId` = `v<major>.<minor>.0`). Do not implement per-stable or per-build re-arming.
- **The CI guard** in `.github/workflows/pre-release-publish.yml` queries Open VSX and fails if the committed version is already published or is not greater than the max published patch on its minor line. The `Microsoft.VisualStudio.Code.PreRelease` marker is stamped EPHEMERALLY around packaging only (the version is never changed).
- After changing `src/package.json`'s version, regenerate the announcement asset with `pnpm generate:announcements` and confirm `pnpm verify:announcement-version` passes (the verify script resolves any patch to its line base, for BOTH channels).
- **No `release/*` branch participates in publishing.** `master` is the only branch that triggers a publish, so the retired `release/v3.88.0-prerelease` + `ovsx-prerelease-publish.yml` path must not be reintroduced (see [`docs/runbooks/branch-hygiene.md`](docs/runbooks/branch-hygiene.md)).
