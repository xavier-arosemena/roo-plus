...
| 2026-09-17 | **Release waiver — `v3.88.5` never published.** `master`'s `src/package.json` is at `3.88.5` (commit `afe35f6a8`), but Open VSX's newest published version is `3.88.4` and `3.88.5` returns HTTP 404 — deliberately. The publish run for `71d621532` (run `35194285835`) failed because the publish guard aborted **fail-closed** on a transient Open VSX registry **HTTP 503**, not on a version problem; the release was deliberately not re-triggered. The next cycle publishes `3.88.6`, so `3.88.5` will be absent from Open VSX **by design** — a future reader must not read the version gap as a failed or incomplete sync. |
