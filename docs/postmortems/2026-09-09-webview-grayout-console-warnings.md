...
- **2026-09-18 — 3.88.7 watch result: the `taskHistory` residual is unfixed, and a SECOND, client-side mechanism is now evidenced.** Fresh-session captures ([2026-09-18 gray-webview](../incidents/2026-09-18-gray-webview.md)) show `ERROR "state" payload 1089–1119KB top[taskHistory=1035–1042KB customModes=56–82KB clineMessages=0KB]` on server B (popup shown), plus `WARN 704–711KB top[taskHistory=621KB]` on server A. **A `git diff` of `src/core/webview`, `src/services`, `src/shared` and `webview-ui/src` between the `3.88.3` release and `3.88.7` is EMPTY** — so "graying more frequently" is **not** a webview code regression; the payload simply grew (1011 → 1042 KB) while the byte-bound fix stayed uncommitted (`fix/taskhistory-byte-bound`). More significant: `INFO Extension host (LocalProcess pid: N) is unresponsive.` **and** `INFO Extension host (Remote) is unresponsive.` fire **together**, while `[host-health]` shows the remote host idle (`elu_ms p50=10 p99≈13 max≤270`, `jitter ≤50`, `cpu_pct 4–16`, `state_serialize_ms p50=0`) — alongside **401s on webview assets (`shellscript-*.js`, `rolldown-runtime-*.js`, `howler-*.js`) on BOTH servers**. A host at 10 ms event-loop lag is not CPU-starved; the shared, stressed component is the **client-side webview resource/transport layer** (postmortem item 5, now with field evidence). That is the mechanism payload bounding cannot fix, and it matches the user-visible signature: only Roo+ webviews gray, and they gray simultaneously across different servers while other webviews render fine. Follow-ups recorded in [`DEBT.md`](../../DEBT.md).
...
- **2026-09-17 (later) — gap G2 fixed and independently verified; capture re-armed on the fixed harness.**
    ...
    - **Correction (2026-09-17) — supersedes the drift line above; it was accurate when written, but not after the fast-forward.** The drift was a _checkout_ artefact, not a repo fact: local `master` was **7 commits behind** `origin/master` (sitting on `71d621532`, the PR #346 merge) while this branch was prepared, so the working tree still read **3.88.5**. `master` has since been **fast-forwarded** (`git merge --ff-only origin/master`) to **`7b4f92882`** (PR #348), where `src/package.json` reads **3.88.6**; the branch carrying this correction (`observability/watch-cycle-3883-capture-watchdog`) then bumps the same minor line to the next committed pre-release patch, **3.88.7** (`pnpm bump:pre-release`; `pnpm verify:announcement-version` passes, and `src/shared/announcements.ts` regenerates with no diff — still keyed to the line base `3.88.0`). The **installed/active build under observation remains 3.88.3**, so the "re-base the next §6 pass on whichever build is actually installed" instruction is unchanged; only the repo-state claim is corrected.
...
- **2026-09-17 — dual-server replay (Pass B follow-up): healthy — no `unresponsive` on either host; `taskHistory` residual now near-ERROR on server B (1006 KB); capture stale again (G2 at runtime).**
    - **Context:** the same operator drove two remote-SSH windows on **different** hosts simultaneously — server A `<remote-A>` (2 vCPU) and server B `<remote-ip>` (4 vCPU); both extension hosts (re)started `~12:08–12:12Z` (A's EH `1485457` exited `12:08:51Z`, replaced by `1522044` `12:08:55Z`). Build: Roo+ **3.88.3** on both.
